...
class Agent:
    def __init__(self):
        self.env = SnakeEnv(width=10, height=10)
        s0, _ = self.env.reset()
...
if __name__ == "__main__":
    ...
    while True:
        iter_no += 1
        EPSILON = EPSILON * np.exp(-EPSILON_DECAY * iter_no)
        ...
        writer.add_scalar("score", np.mean(test_scores), iter_no)
        ...
        if test_reward > best_reward:
            ...
            print("Scores: mean=%.1f, max=%.1f, min=%.1f" % (np.mean(test_scores), np.max(test_scores), np.min(test_scores)))
            print("Episode length: mean=%.1f, max=%.1f, min=%.1f" % (np.mean(test_lengths), np.max(test_lengths), np.min(test_lengths)))
